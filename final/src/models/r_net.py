    ...
    def forward(self, *tensors):
        ...
        for tensor in tensors:
            ...
            if torch.cuda.is_available():
                word_level_embed = word_level_embed.cuda()
...
class Model(nn.Module):
    def __init__(self, args, char_embedding_config, word_embedding_config, sentence_encoding_config,
        ...
        USE_TF = True
        if USE_APPEARANCE:
            # embedding size add one
            new_embedding_size = self.embedding.embedding_size+1
            if USE_POS:
                new_embedding_size += 66
            if USE_TF:
                new_embedding_size += 1
            self.r_net = RNet(args, new_embedding_size, char_embedding_config, word_embedding_config,
...
class RNet(nn.Module):
    ...
    def forward(self, question: Documents, passage: Documents, embedded_question, embedded_passage):
        ...
        begin, end = self.pointer_output(question_pad_in_passage_sorted_order,
                                         question_mask_in_passage_sorted_order,
                                         #pad_packed_sequence(paired_passage_pack)[0],
